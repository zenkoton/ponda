...
/**
 * [ponda P1 patch] 原生读取 ponda 全局状态定位当前环境 agent dir。
 * 优先级：ENV_AGENT_DIR 显式覆盖 > ponda state.json > pi 默认目录。
 * 坏 state / 环境目录缺失时静默回落默认（不阻塞 pi 启动）。
 */
export function getPondaAgentDir(): string | null {
	const home = process.env.PONDA_HOME ?? join(homedir(), ".ponda");
	const stateFile = join(home, "state.json");
	try {
		const state = JSON.parse(readFileSync(stateFile, "utf8")) as { activeEnv?: string | null };
		if (typeof state.activeEnv !== "string" || state.activeEnv.length === 0) return null;
		const dir = join(home, "envs", state.activeEnv);
...
export function getAgentDir(): string {
	...
	const pondaDir = getPondaAgentDir(); // [ponda P1]
	if (pondaDir !== null) {
		return pondaDir;
	}
