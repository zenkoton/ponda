 ...
 */
import { cpSync, existsSync, mkdirSync, readdirSync, readFileSync, readlinkSync, renameSync, rmSync, writeFileSync, } from "node:fs";
import { join } from "node:path";
import { ENV_SUBDIRS, paths, pondaHome } from "./paths.js";
import { planRender, renderEnv } from "./render.js";
import { EnvNotFoundError, resolveEnv } from "./resolve.js";
import { validateEnvName, validateManifestInput } from "./validate.js";
...
export class EnvStore {
    ...
    readManifest(name) {
        ...
        try {
            ...
            throw new ValidationError(`manifest.json 解析失败（${name}）：${e.message}`);
        }
    }
    writeManifest(name, input) {
        const errors = validateManifestInput(input);
        if (errors.length > 0)
            throw new ValidationError(`manifest 校验失败（${name}）：\n  - ${errors.join("\n  - ")}`);
        const file = paths.envManifest(this.home, name);
    ...
    saveManifest(input) {
        ...
        if (name === undefined)
            throw new ValidationError("saveManifest 需要 input.name");
    ...
    create(opts) {
        ...
        if (this.exists(opts.name))
            throw new ValidationError(`环境已存在：${opts.name}`);
        const base = opts.base ?? "default";
        if (base !== opts.name && !(base === "default" && !this.exists("default"))) {
            if (!this.exists(base))
                throw new EnvNotFoundError(base);
        }
        const input = {
            schemaVersion: 1,
            name: opts.name,
    ...
    remove(name, opts = {}) {
        ...
        if (referrers.length > 0 && !opts.force) {
            throw new ValidationError(`环境 ${name} 被以下环境继承：${referrers.join(", ")}；使用 --force 将其改继 ${name} 的父级`);
        }
        const removedBase = this.readManifest(name)?.base;
        for (const r of referrers) {
            const m = this.readManifest(r);
            const next = { ...m };
            if (removedBase === undefined || removedBase === "default")
                delete next.base;
            else
                next.base = removedBase;
    ...
    rename(oldName, newName) {
        if (!this.exists(oldName))
            throw new EnvNotFoundError(oldName);
        const nameErrors = validateEnvName(newName);
        if (nameErrors.length > 0)
            throw new ValidationError(nameErrors.join("；"));
        if (this.exists(newName))
            throw new ValidationError(`环境已存在：${newName}`);
        const m = this.readManifest(oldName);
        m.name = newName;
    ...
    list() {
        ...
        return this.names().map((name) => {
            ...
            if (existsSync(actFile))
                lastActivatedAt = readFileSync(actFile, "utf8").trim();
            return {
                name,
    ...
    exportEnv(name, outDir) {
        ...
        for (const sub of ["skills", "extensions", "themes", "prompts"]) {
            ...
            for (const e of entries) {
                ...
                if (e.isSymbolicLink())
                    continue; // 池内共享资源由 links.json 记录
                cpSync(from, join(outDir, sub, e.name), { recursive: true });
            }
        }
        // 受管软链接清单
        const links = [];
        for (const sub of ["skills", "extensions", "themes"]) {
            const src = join(envDir, sub);
            if (!existsSync(src))
                continue;
        ...
        writeAtomic(join(outDir, "ponda-export.json"), `${JSON.stringify({ name, exportedAt: new Date().toISOString(), links }, null, "\t")}\n`);
    }
    importEnv(dir, opts = {}) {
        const manifestFile = join(dir, "manifest.json");
        if (!existsSync(manifestFile))
            throw new ValidationError(`导入目录缺少 manifest.json：${dir}`);
        const input = JSON.parse(readFileSync(manifestFile, "utf8"));
        const name = opts.name ?? input.name;
        if (name === undefined)
            throw new ValidationError("manifest 缺少 name 且未提供 --name");
        if (this.exists(name))
            throw new ValidationError(`环境已存在：${name}`);
        ...
        for (const sub of ["skills", "extensions", "themes", "prompts"]) {
            ...
            if (existsSync(src))
                cpSync(src, join(envDir, sub), { recursive: true });
        }
        const res = this.resolve(name);
        renderEnv(this.home, name, res.effective);
        return res;
        ...
        for (const name of this.names()) {
            try {
                ...
                for (const w of plan.warnings)
                    report.push({ level: "warn", message: `${name}: ${w}` });
                // 渲染漂移：计划文件与磁盘内容比对
                const envDir = paths.env(this.home, name);
                ...
                report.push({ level: "error", message: `${name}: ${e.message}` });
            }
        }
        const state = this.readState();
...
function readlinkRaw(p) {
    return readlinkSync(p);
}
function diffRecords(a, b, prefix) {
    const out = [];
    ...
    for (const k of keys) {
        ...
        if (typeof va === "object" &&
            ...
            vb !== null &&
            !Array.isArray(vb)) {
            out.push(...diffRecords(va, vb, path));
            continue;
        }
