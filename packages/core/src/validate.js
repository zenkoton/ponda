 ...
 */
import { BUILTIN_TOOLS, PRIVILEGES, PROVIDER_APIS } from "./types.js";
...
function checkSelectorList(list, field, errors) {
    ...
    for (const item of list) {
        ...
        if (isPlainObject(item)) {
            ...
            if (keys.includes("name") && typeof item.name === "string") {
                if (keys.includes("version") && typeof item.version === "string")
                    continue;
                if (keys.includes("path") && typeof item.path === "string")
                    continue;
            }
        }
...
export function validateManifestInput(input) {
    ...
    if (input.skills !== undefined)
        checkSelectorList(input.skills, "skills", errors);
    if (input.extensions !== undefined)
        checkSelectorList(input.extensions, "extensions", errors);
    if (input.themes !== undefined)
        checkSelectorList(input.themes, "themes", errors);
        ...
        if (m.providers !== undefined) {
            if (!isPlainObject(m.providers)) {
                ...
                for (const [k, v] of Object.entries(m.providers)) {
                    ...
                    else if (!PROVIDER_APIS.includes(v.api)) {
    ...
    if (input.privileges !== undefined) {
        ...
        if (p.sandbox !== undefined) {
            if (!isPlainObject(p.sandbox)) {
                ...
                if (p.sandbox.outsideWorkspace !== undefined &&
    ...
    if (input.runtime !== undefined) {
        ...
        if (r.contextStrategy !== undefined &&
        ...
        if (r.maxParallelSubagents !== undefined &&
...
/** 选择器的名字（含 "!" 排除前缀则原样返回） */
export function selectorName(s) {
    if (typeof s === "string")
        return s;
    return s.name;
}
/** 是否为排除选择器（"!name" 形态） */
export function isExclusionSelector(s) {
    return selectorName(s).startsWith("!");
}
/** 排除选择器去掉 "!" 后的目标名 */
export function exclusionTarget(s) {
