...
/** daemon 启动体（源码直跑与打包形态 `_daemon` 子命令共用） */
export async function runDaemonMain(opts: { env: string; home: string; idleMs?: number }): Promise<void> {
	const { env, home, idleMs } = opts;
	const logFile = process.env.PONDA_DAEMON_LOG;
...
// 源码直跑入口（node main.ts --env ...）；打包形态经 bin.ts `_daemon` 分派。
// 参数解析与校验只在此处做——本模块被 bin.ts import 时顶层不得有副作用。
if (process.argv[1]?.endsWith("main.ts") === true) {
	...
	await runDaemonMain({ env, home, idleMs });
}
