...
import { AdaptivePublisher } from "../src/env/utils/adaptive-publisher.ts";
...
describe("AdaptivePublisher", () => {
	...
	it("advances its baseline without publishing or delaying an undefined update", () => {
		vi.useFakeTimers();
		vi.setSystemTime(0);
		let value = "a";
		...
		const publisher = new AdaptivePublisher({
			...
			update: (previous, current) => {
				...
				return current === "skip" ? undefined : current;
			},
			measure: () => 1,
		...
		expect(previousValues).toEqual([undefined, "skip"]);
	});

	it("reports trailing publication failures and cancels pending work on dispose", () => {
		vi.useFakeTimers();
		vi.setSystemTime(0);
		let value = 0;
		...
		const publisher = new AdaptivePublisher({
			...
			publish: (update) => {
				...
				if (update === 2) throw new Error("trailing failed");
			},
		...
		publisher.markDirty();
		vi.advanceTimersByTime(100);
		expect(errors).toEqual([new Error("trailing failed")]);
